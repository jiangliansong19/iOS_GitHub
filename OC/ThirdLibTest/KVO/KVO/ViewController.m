...
- (void)viewDidLoad {
    ...
    self.Persons = [Persons new];
    [self.Persons addObserver:self forKeyPath:@"address" options:NSKeyValueObservingOptionOld| NSKeyValueObservingOptionNew context:nil];
...
- (IBAction)buttonAction2:(UIButton *)sender {
    [self.Persons willChangeValueForKey:@"address"];
    [self.Persons didChangeValueForKey:@"address"];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSKeyValueChangeKey,id> *)change context:(void *)context {
    if ([keyPath isEqualToString:@"name"]) {
        ...
        NSLog(@"self.Persons.address = %@", _nameTF.text);
    }
}
